// Verison funcionando al 24/07/25. Crea boton de whatsapp, 
// carga formulario de google sheets le cambia el nombre y crea el link desde sheets.

// Mejoras pendientes:
//  Modo obscuro, Cambiar nobre de pdf a (nombre+rut), cambiar rango de edad por fecha de nacimiento, 
//  cambiar de 3 a 4 steps y ordenarlos, optimizarlo para moviles.
...
const COLUMN_HEADERS = [
  'FechaEnvio','Estado','IP','Fuente','Campaña','Nombre','Rut','Email',
  'Teléfono','Edad','Est. Civil','Comuna','Región','Sist. Salud',
  'Isapre','Anualidad','Cargas','Estatura','Peso','Interés',
  'Costo Plan','Evaluar AFP','AFP','Archivo PDF','Contacto WA','Estado WA'
];
...
function doPost(e) {
  ...
  try {
    ...
    Logger.log("📥 Parámetros recibidos: " + JSON.stringify(data));

    const fuente = (data.fuente || '').toLowerCase();
    ...
    try {
      ...
      const nombreArchivo = e.parameter.filename || Utilities.formatDate(new Date(), "America/Santiago", "ddMMyyyy_HHmmss") + ".pdf";
      ...
      Logger.log("✅ PDF subido con éxito: " + fileId);
    } catch (err) {
      Logger.log("❌ Error al procesar archivo base64: " + err);
    }
    ...
    const row = COLUMN_HEADERS.map(h => {
      switch (h) {
        ...
        case 'Interés': return data.interes || '';
    ...
    const cargas = data.num_cargas > 0 ? `• Cargas: ${data.num_cargas}\n` : '';
    const interes = data.interes || '';
    const costo = data.rango_costo || '';
    const afp = data.evaluar_afp === 'Si' ? `• Me interesa AFP\n• AFP Actual: ${data.afp_actual || ''}\n` : '';
    const msg = `Hola, ${nombre}, te saludamos de PlanesPro:\n\n` +
      `• Rut: ${data.rut || ''}\n` +
      `• Teléfono: ${telefono}\n` +
      `• Comuna: ${comuna}\n` +
      `• Región: ${region}\n` +
      `• Sistema de Salud: ${sistema}${isapre}\n` +
      cargas +
      `• Interés: ${interes}\n` +
      `• Costo plan actual: ${costo}\n` +
      afp +
      `\nGracias por tu confianza. Pronto te entregamos tu análisis.`;
    ...
    return ContentService.createTextOutput(JSON.stringify({ result: 'success', message: 'Formulario enviado con éxito.' }))
...
function prepararContactoWhatsApp() {
  ...
  const cargasTxt = d['Cargas'] > 0 ? `• Cargas: ${d['Cargas']}\n` : '';
  const isapreTxt = d['Sist. Salud'] === 'Isapre' && d['Isapre'] ? ` (${d['Isapre']})` : '';
  const afpTxt = d['Evaluar AFP'] === 'Si' ? `• Me interesa AFP\n• AFP Actual: ${d['AFP']}\n` : '';
  ...
  const msg = `Hola, ${d['Nombre']}, te saludamos de PlanesPro:\n\n` +
    ...
    `• Comuna: ${d['Comuna']}\n` +
    `• Región: ${d['Región']}\n` +
    `• Sistema de Salud: ${d['Sist. Salud']}${isapreTxt}\n` +
    cargasTxt +
    `• Interés: ${d['Interés']}\n` +
    `• Costo plan actual: ${d['Costo Plan']}\n` +
    afpTxt +
    `\nGracias por tu confianza. Pronto te entregamos tu análisis.`;

  const tel = `56${String(d['Teléfono']).replace(/\D/g, '')}`;
  ...
  ui.alert('✅ Botón de WhatsApp creado y marcado como "Contactado".');
}
...
function sendConfirmationEmail(d) {
  ...
  <div style="font-family:Arial;max-width:600px;margin:auto">
    ...
    <div style="padding:20px;background:#f9f9f9">
      ...
      <p>Recibimos tu solicitud. Aquí tus datos:</p>
      ...
      ${cargas}
      <p><strong>Interés:</strong> ${d.interes}</p>
      <p><strong>Costo plan actual:</strong> ${d.rango_costo}</p>
      ${afpSection}
      <p>En breve recibirás tu evaluación.</p>
  ...
  MailApp.sendEmail({
    to: d.email,
    subject: "Tu análisis de Plan de Salud está en proceso",
    htmlBody: html,
    name: "PlanesPro"
  });
}
